...
async def seed_full_menu():
    ...
    async with pool.acquire() as conn:
        ...
        # --- Create menu_item_availability_schedule table ---
        await conn.execute("""
        ...
        for item in MENU_ITEMS:
            ...
            if cat_id:
                await conn.execute("DELETE FROM menu_item_category_mapping WHERE menu_item_id = $1", menu_item_id)
                await conn.execute("""
            ...
            if meal_type_id:
                await conn.execute("DELETE FROM menu_item_availability_schedule WHERE menu_item_id = $1", menu_item_id)
                await conn.execute("""
