...
class AddToCartTool(ToolBase):
    ...
    async def _execute_impl(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            redis = RedisService()
            cart_key = f"cart:{session_id}"
            cart_data = redis.get(cart_key) or {"items": []}
            ...
            redis.set(cart_key, cart_data, ttl=3600)  # 1 hour TTL
    ...
    async def _get_alternatives_message(self, item_id: str, item_name: str) -> Optional[str]:
        ...
        try:
            ...
            from app.tools.database.menu_ai_tools import FindSimilarItemsTool
...
class RemoveFromCartTool(ToolBase):
    ...
    async def _execute_impl(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            redis.set(cart_key, cart_data, ttl=3600)
    ...
    async def _execute_impl(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            if not keep_existing:
                ...
                deleted_count = redis.delete(cart_key)

                # Verify cart is actually gone
                verification = redis.get(cart_key)
