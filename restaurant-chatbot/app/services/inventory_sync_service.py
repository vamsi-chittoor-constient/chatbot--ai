...
class InventorySyncService:
    ...
    async def sync_full(self):
        ...
        # NOTE: Commented out - MenuItem model doesn't have inventory tracking fields
        logger.info("full_sync_skipped - inventory tracking fields not in MenuItem model")
        ...
        #             select(MenuItem).where(MenuItem.menu_item_in_stock == True)
        #         )
        #         all_items = result.scalars().all()

        #         item_ids = [item.menu_item_id for item in all_items]
        ...
        #             redis_data = sync_data.get(item.menu_item_id)
        #             if redis_data:
    ...
    async def sync_items(self, item_ids: List[str], reason: str = "event"):
        ...
        # NOTE: Commented out - MenuItem model doesn't have inventory tracking fields
        logger.info("selective_sync_skipped - inventory tracking fields not in MenuItem model")
        self.reset_sync_timer()
        ...
        #         result = await db_session.execute(
        #             select(MenuItem).where(MenuItem.menu_item_id.in_(item_ids))
        #         )
    ...
    async def sync_after_order(self, order_items: List[Dict]):
        ...
        # NOTE: Commented out - MenuItem model doesn't have inventory tracking fields
        logger.info("order_completion_sync_skipped - inventory tracking fields not in MenuItem model")
        self.reset_sync_timer()
        ...
        #             select(MenuItem).where(MenuItem.menu_item_id.in_(item_ids))
        #         )
        #         items = {item.menu_item_id: item for item in result.scalars().all()}
    ...
    async def sync_after_manager_update(self, item_id: str, new_total_stock: int):
        ...
        # NOTE: Commented out - MenuItem model doesn't have inventory tracking fields
        logger.info("manager_update_sync_skipped - inventory tracking fields not in MenuItem model")
        self.reset_sync_timer()
    ...
    async def sync_on_startup(self):
        ...
        # NOTE: Commented out - MenuItem model doesn't have inventory tracking fields
        # (total_stock_quantity, reserved_quantity, available_quantity)
        logger.info("startup_sync_skipped - inventory tracking fields not in MenuItem model")
        self._last_sync_time = datetime.now()
