...
# =============================================================================
# DETERMINISTIC PAYMENT API — bypasses LLM agent entirely
# =============================================================================

@router.get("/payment/order-summary")
async def get_order_summary(
    session_id: str = Query(..., description="Session ID with pending payment")
):
    """
    Get order summary for the payment page.

    Returns order details from Redis payment state so the standalone
    payment page can display items, amount, and payment options.
    """
    from app.services.payment_state_service import get_payment_state
    ...
    return {
        ...
        "order_type": payment_state.get("order_type", "take_away"),
        "subtotal": payment_state.get("subtotal", 0),
        "packaging_charges": payment_state.get("packaging_charges", 0),
        "step": payment_state.get("step"),
    }


@router.post("/payment/select-method")
async def select_payment_method_api(request: Request):
    """
    Deterministic payment method selection — no LLM involved.

    Called directly by the PaymentMethodCard buttons and the standalone
    payment page. Runs the payment workflow with the selected method.
    """
    ...
    method = body.get("method")  # "online" | "cash" | "card_at_counter"

    if not session_id or not method:
        raise HTTPException(status_code=400, detail="session_id and method are required")

    # Normalize button actions to method values
    method_map = {
        "pay_online": "online",
        "pay_cash": "cash",
    ...
    if payment_state.get("step") != PaymentStep.SELECT_METHOD.value:
        raise HTTPException(
            status_code=409,
            detail=f"Payment is not awaiting method selection (current step: {payment_state.get('step')})"
        )

    logger.info(
        "payment_method_selected_via_api",
        session_id=session_id,
        method=normalized,
        order_id=payment_state["order_id"],
    )
    ...
    try:
        result = await run_payment_workflow(
            session_id=session_id,
            order_id=payment_state["order_id"],
            amount=payment_state.get("amount", 0),
        ...
        logger.error("payment_select_method_failed", error=str(e), session_id=session_id, exc_info=True)
