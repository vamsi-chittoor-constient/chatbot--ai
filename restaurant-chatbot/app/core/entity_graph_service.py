...
- Pronoun resolution (tracks conversation referents)
- Entity remembrance (user preferences, history)
- Multi-hop reasoning ("my usual order")
...
class EntityGraphService:
    ...
    - CONVERSATION: Conversation context (message_id, referent)
    - CONVERSATION_TURN: Active conversation state (sub_intent, entities, collection_step)

    Edge types:
    - HAS_CART: User -> Cart (current active cart)
    - CONTAINS: Cart -> MenuItem (cart items with quantity)
    - LAST_MENTIONED: User -> MenuItem (for pronoun resolution)
    - PREFERS: User -> Preference (dietary, spice level, etc.)
    - ORDERED: User -> Order (order history)
    - FAVORITE: User -> MenuItem (favorite items)
    - HAS_ACTIVE_INTENT: User -> ConversationTurn (current conversation state)
    """
    ...
    def set_last_mentioned_item(self, user_id: str, item_name: str, menu_item_id: Optional[str] = None) -> None:
        """
        Track last mentioned item for pronoun resolution.
    ...
    def get_last_mentioned_item(self, user_id: str) -> Optional[Dict[str, Any]]:
        """Get last mentioned item (for pronoun resolution)."""
        user_node = f"user:{user_id}"
