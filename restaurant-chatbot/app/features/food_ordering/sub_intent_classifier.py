...
async def classify_sub_intent(
    ...
    # This is the source of truth - DO NOT rely on LangGraph state
    ...
    if user_id:
        try:
            ...
            logger.warning("Failed to load from external storage, falling back to LangGraph state", error=str(e))

    # Fallback to LangGraph state ONLY if external storage returned nothing
    ...
    if messages:
        ...
        # Note: In LangGraph, the current message might already be in state["messages"]
